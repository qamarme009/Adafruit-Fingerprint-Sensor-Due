...
Adafruit_Fingerprint_Due::Adafruit_Fingerprint_Due() 
{
  thePassword = 0;
  theAddress = 0xFFFFFFFF;
}

void Adafruit_Fingerprint_Due::begin(uint16_t baudrate) 
{
  Serial1.begin(baudrate);
}

boolean Adafruit_Fingerprint_Due::verifyPassword(void) {
...
uint8_t Adafruit_Fingerprint_Due::getImage(void) {
...
uint8_t Adafruit_Fingerprint_Due::image2Tz(uint8_t slot) {
...
uint8_t Adafruit_Fingerprint_Due::createModel(void) {
...
uint8_t Adafruit_Fingerprint_Due::storeModel(uint16_t id) {
...
uint8_t Adafruit_Fingerprint_Due::emptyDatabase(void) {
...
uint8_t Adafruit_Fingerprint_Due::fingerFastSearch(void) {
  fingerID = 0xFFFF;
  confidence = 0xFFFF;
...
uint8_t Adafruit_Fingerprint_Due::getTemplateCount(void) {
...
void Adafruit_Fingerprint_Due::writePacket(uint32_t addr, uint8_t packettype, 
...
#else
  Serial1.print((uint8_t)(FINGERPRINT_STARTCODE >> 8), BYTE);
  Serial1.print((uint8_t)FINGERPRINT_STARTCODE, BYTE);
  Serial1.print((uint8_t)(addr >> 24), BYTE);
  Serial1.print((uint8_t)(addr >> 16), BYTE);
  Serial1.print((uint8_t)(addr >> 8), BYTE);
  Serial1.print((uint8_t)(addr), BYTE);
  Serial1.print((uint8_t)packettype, BYTE);
  Serial1.print((uint8_t)(len >> 8), BYTE);
  Serial1.print((uint8_t)(len), BYTE);
#endif
...
#else
    Serial1.print((uint8_t)(packet[i]), BYTE);
#endif
#ifdef FINGERPRINT_DEBUG
...
#else
  Serial1.print((uint8_t)(sum>>8), BYTE);
  Serial1.print((uint8_t)sum, BYTE);
#endif
}


uint8_t Adafruit_Fingerprint_Due::getReply(uint8_t packet[], uint16_t timeout) {
